...
public class AlertTask extends BukkitRunnable {
    ...
    public AlertTask(InvictusSync plugin) {
        this.plugin = plugin;
    }
    ...
    public void run() {
        ...
        try {
            ...
            double currentTps = Math.round(tps[0] * 10.0) / 10.0;
            if (currentTps < tpsThreshold) {
                if (canAlert("tps", cooldownMs)) {
                    String msg = plugin.getMsg("alert-tps")
                        .replace("{tps}", String.valueOf(currentTps));
                    notifyStaff(msg);
                    String json = String.format(
                        "{\"level\":\"SEVERE\",\"message\":\"[AlertTask] TPS bajo: %s\",\"logger\":\"InvictusSync\",\"ts\":%d}",
                        currentTps, System.currentTimeMillis()
                    );
                    plugin.getWorkerClient().post("/mc/console/log", "{\"logs\":[" + json + "]}");
                }
            }
        ...
        try {
            ...
            if (max > 0) {
                ...
                if (percent >= ramThreshold) {
                    if (canAlert("ram", cooldownMs)) {
                        String msg = plugin.getMsg("alert-ram")
                            .replace("{percent}", String.valueOf(percent));
                        notifyStaff(msg);
                        String json = String.format(
                            "{\"level\":\"SEVERE\",\"message\":\"[AlertTask] RAM alta: %d%%\",\"logger\":\"InvictusSync\",\"ts\":%d}",
